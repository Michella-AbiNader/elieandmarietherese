// src/config.js
export const invitationConfig = {
  section1: {
    title: "دعوة زفاف", // top title
    parents: {
      brideFather: "السيد سليمان رعد",
      brideMother: "و زوجته لوريت رعد",
      groomFather: "السيد جهاد سلمان",
      groomMother: "و زوجته سمية آدم",
    },
    sentence1: "يتشرفون بدعوتكم لحضور حفل إكليل  إبنيهما", // centered sentence under parents
    groomName: "إلياس", 
    brideName: "ماري تيريز",
    sentence2: "و ذلك مع دقات أجراس الفرح في تمام الساعة 6 من مساء يوم", // final centered sentence
  },
  coupleNames: "إيلي و ماري تيريز",
  date: "12 اب 2026",
  location: "فندق الريتز كارلتون، بيروت",
  backgroundColor: "#fce5cd",
  textColor: "#5d4037",
  fontFamily: "Cairo, sans-serif",
  buttonColor: "#d32f2f",
  quote: "\"اِثْنَانِ خَيْرٌ مِنْ وَاحِدٍ، لأَنَّ لَهُمَا أُجْرَةً لِتَعَبِهِمَا صَالِحَةً. لأَنَّهُ إِنْ وَقَعَ أَحَدُهُمَا يُقِيمُهُ رَفِيقُهُ. وَوَيْلٌ لِمَنْ هُوَ وَحْدَهُ إِنْ وَقَعَ، إِذْ لَيْسَ ثَانٍ لِيُقِيمَهُ\"، (سفر الجامعة 4: 9–10)",
  weddingDate: "2026-09-12T18:00:00",
  weddingDate_String: "السبت ١٢ أيلول ٢٠٢٦",
  ceremony: {
    title: "مراسم الإكليل",
    name: "كنيسة مار يعقوب السروجي",
    city: "Sabtiyeh",
    time: "الساعة 6:00 مساءً",
    mapLink: "https://maps.app.goo.gl/uAWoqb1R25aY2c4BA?g_st=aw",
  },
  venue: {
    title: "تليها سهرة عشاء واحتفال في",
    name: "نادي الرتباء",
    city: "الفياضية",
    mapLink: "https://maps.app.goo.gl/sMLiyyquqrFMoAYt6?g_st=aw",
  },
   giftRegistry: {
    title: "قائمة الهدايا",
    description: "حبكم ووجودكم معنا هما كل ما نتمناه في يومنا الخاص. لمن يرغب، قائمة الهدايا متاحة.",
    accountLabel: "رقم حساب wish:",
    accountNumber: "111111111-03",
  },
   rsvp: {
    title: "كونوا ضيوفنا",
    deadline: "يرجى تأكيد حضوركم قبل: ...2  ",
    namePlaceholder: "الاسم الكامل",
    attendingYes: "سأحضر",
    attendingNo: "لن أتمكن من الحضور",
    submitText: "إرسال",
    numebrOfGuests: "عدد الضيوف",
  },
};